...
class PrescriptionEligibilityRepo(Protocol):
...
class SupabasePrescriptionEligibilityRepo:
    """Reads patient/appointment/prescription/task data from Supabase using the service role key."""
    ...
    def get_appointments(self, patient_id: str) -> list[dict[str, Any]]:
        response = self._client.table("appointments").select("*").eq("patient_id", patient_id).execute()
        return response.data or []

    def get_prescriptions(self, patient_id: str) -> list[dict[str, Any]]:
        response = self._client.table("prescriptions").select("*").eq("patient_id", patient_id).execute()
        return response.data or []
