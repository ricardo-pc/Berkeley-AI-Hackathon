...
export default function PatientHub() {
  ...
  const latestInr = patient.inrHistory[0];
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500">
          Last visit {patient.lastVisit} · No outstanding alerts acknowledged
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Panel title="Active Problems">
          {patient.problems.length ? (
            ...
            <p className="text-xs text-slate-400">No active problems on file.</p>
          )}
        </Panel>
        ...
        <Panel title="Allergies / Intolerances">
          {patient.allergies.length ? (
            ...
            <p className="text-xs text-slate-400">No Known Drug Allergies.</p>
          )}
        </Panel>
        ...
        {latestInr ? (
          <Panel title="Anticoagulation Snapshot">
            <div className="text-xs text-slate-700">
              ...
              <Link href={`${base}/coumadin`} className="mt-2 inline-block text-sky-700 underline">
          ...
          <Panel title="Care Summary">
            <p className="text-xs text-slate-500">
              No anticoagulation therapy on file. Recent activity and outstanding
              requests for this patient would appear here.
            </p>
          </Panel>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Panel
          ...
          right={
            <Link href={`${base}/medications`} className="text-[11px] text-sky-700 underline">
              Manage / Refill
            </Link>
          }
        >
        ...
        <Panel
          ...
          right={
            <Link href={`${base}/appointments`} className="text-[11px] text-sky-700 underline">
              Schedule
            </Link>
          }
        >
